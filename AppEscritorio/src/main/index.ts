import { app, BrowserWindow, ipcMain } from 'electron'; // Agregar ipcMain
import { join } from 'path';
import dgram, { RemoteInfo } from 'dgram';
import { createServer, Socket, createConnection } from 'net'; // Agregar createConnection
import { createWriteStream, createReadStream } from 'fs'; // Agregar createReadStream
...
function iniciarServidorDeDescubrimientoUdp(): void {
  try {
    ...
    servidorUdp.bind(PUERTO_DESCUBRIMIENTO, () => {
      ...
      // ✨ LO NUEVO: Activar el permiso de broadcast para que la red nos vea
      servidorUdp.setBroadcast(true);

      // ✨ LO NUEVO: Enviar un Beacon (latido) cada 2 segundos a toda la red local
      setInterval(() => {
        const mensajeBeacon = Buffer.from(JSON.stringify({
          alias: 'PC de Julieta', 
          tipo: 'Escritorio'
        }));
        
        servidorUdp.send(mensajeBeacon, 0, mensajeBeacon.length, PUERTO_DESCUBRIMIENTO, '255.255.255.255');
      }, 2000);
    });
  } catch (error) {
...
// ==========================================
// CLIENTE TCP: ENVÍO DE ARCHIVOS
// ==========================================
function iniciarServicioDeEnvioTcp(): void {
  ipcMain.on('enviar-archivos', (_evento, ipDestino: string, archivos: any[]) => {
    archivos.forEach((archivo) => {
      try {
        // 1. Nos conectamos a la IP del dispositivo detectado
        const clienteTcp = createConnection({ port: PUERTO_TRANSFERENCIA, host: ipDestino }, () => {
          console.log(`🔗 Conectado a ${ipDestino}. Iniciando envío de: ${archivo.name}`);
          
          // 2. Enviamos el "Aviso" (Metadatos en JSON)
          const metadatos = JSON.stringify({ nombre: archivo.name, peso: archivo.size });
          clienteTcp.write(metadatos);

          // 3. Esperamos medio segundo para que el otro lado lea el JSON y abrimos los Streams
          setTimeout(() => {
            // Cumpliendo la rúbrica: Leemos el archivo del disco en fragmentos (chunks)
            const streamDeLectura = createReadStream(archivo.path, { highWaterMark: 64 * 1024 }); 
            
            // "Bombeamos" los fragmentos directo a la red (no bloqueante)
            streamDeLectura.pipe(clienteTcp);

            streamDeLectura.on('end', () => {
              console.log(`✅ Envío de ${archivo.name} completado exitosamente.`);

              if (ventanaPrincipal) {
                ventanaPrincipal.webContents.send('envio-completado', archivo.name);
              }
            });
          }, 500);
        });

        clienteTcp.on('error', (errorDeConexion) => {
          console.error("❌ Error al intentar enviar el archivo:", errorDeConexion);
        });
      } catch (error) {
        console.error("No se pudo iniciar el cliente TCP:", error);
      }
    });
  });
}
