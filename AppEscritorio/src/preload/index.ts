  ...
  servidorWs.on('connection', (socket) => {
    ...
    socket.on('message', (datos: Buffer) => {
      if (flujoEscritura) {
        ...
        let tiempoEstimado = `${segundosRestantes}s`;
        if (segundosRestantes > 60) tiempoEstimado = `${Math.floor(segundosRestantes/60)}m ${segundosRestantes%60}s`;

        ventanaPrincipal?.webContents.send('progreso-transferencia', {
          nombre: metadatos?.nombre || 'Archivo', porcentaje, velocidad: velocidadNum.toFixed(2), esRecepcion: true, tiempoEstimado
        });
        return;
      }
      ...
      try {
        ...
        if (mensaje.accion === ACCION_SOLICITAR) {
          ...
          const procesarAceptacion = (aceptada: boolean) => {
            if (aceptada) {
              ...
              if (fs.existsSync(rutaDestino)) {
                  const choice = dialog.showMessageBoxSync(ventanaPrincipal!, {
                      type: 'question',
                  ...
                  if (choice === 2) { // Omitir
                      ...
                      socket.close();
                      return;
                  } else if (choice === 1) { // Mantener ambos
          ...
          if (!ventanaPrincipal?.isFocused() && Notification.isSupported()) {
              ...
              notificacion.on('action', (_event, index) => {
                  if (index === 0) procesarAceptacion(true);
                  else procesarAceptacion(false);
              });
              notificacion.show();
          } else {
    ...
    socket.on('close', () => {
      if (flujoEscritura) {
        ...
        flujoEscritura.end();
        ventanaPrincipal?.webContents.send('progreso-transferencia', {
          nombre: metadatos?.nombre || 'Archivo', porcentaje: "100.00", velocidad: "0.00", esRecepcion: true, ruta: rutaFinal, tiempoEstimado: "0s"
        });
      }
    });
  });
...
ipcMain.on('iniciar-envio-archivos', (_e, { direccionIp, archivos }) => {
  archivos.forEach((archivo: any) => {
    ...
    if (esCelular) {
        ...
        ventanaPrincipal?.webContents.send('progreso-transferencia', { nombre: archivo.name, porcentaje: "100.00", velocidad: "Enviado link", esRecepcion: false, tiempoEstimado: "0s" });
        return; 
    }
    ...
    socket.on('message', (datos: Buffer) => {
      ...
      if (respuesta.accion === ACCION_ACEPTAR) {
        ...
        flujoLectura.on('data', (chunk) => {
          ...
          ventanaPrincipal?.webContents.send('progreso-transferencia', {
            nombre: archivo.name, porcentaje: ((bytesEnviados / archivo.size) * 100).toFixed(2), velocidad: (bytesEnviados / (1024 * 1024) / (duracion || 1)).toFixed(2), esRecepcion: false, tiempoEstimado
          });
        });
